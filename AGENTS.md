...
6. Extensions must be created in `src/directus/extensions` They are then compiled with `npm install` and `npm run build`.
...
14. However, if in a git worktree where the project root dir folder contains the word "agent", always commit and push and open a PR.
