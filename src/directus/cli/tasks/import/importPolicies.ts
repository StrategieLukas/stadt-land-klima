...
interface ImportPoliciesOptions {
  verbose?: boolean;
  remove?: boolean;
  overwrite?: boolean;
}

async function importPolicies(src: string, options: ImportPoliciesOptions = { verbose: false, remove: false, overwrite: false }): Promise<void> {
  const client = createDirectusClient();

  try {
    // Get existing policies and permissions
    const existingPolicies: Policy[] = await client.request(
      readPolicies({ limit: -1 })
    );
    const existingPermissions: Permission[] = await client.request(readPermissions({ limit: -1 }));

    // Read policy YAML files
    const policies: Policy[] = readYamlFiles(path.join(src));
    ...
    const policyPermissionMap = new Map<string, any[]>(); // policyName -> permissions
    ...
    for (const policy of policies) {
      ...
      for (const permission of policyPermissions) {
        ...
        const permissionWithPolicy: Permission = { ...permission, policy: policyObj.id };
