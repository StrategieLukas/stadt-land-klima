...
export default {
  ...
  handler: (router: Router): void => {
     ...
     */
    router.get('/search', async (req: Request<{}, {}, {}, SearchParams>, res: Response<UnsplashSearchResponse>) => {
      const q = String(req.query.q || '').trim();
      if (!q) {
        return res.status(400).json({ error: 'Missing query parameter "q"' } as any);
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          return res.status(response.status).json({ error: text } as any);
        }
        ...
        return res.status(500).json({ error: 'Upstream request failed' } as any);
      }
    });
     ...
     */
    router.post('/trigger-download', async (req: Request<{}, {}, TriggerDownloadBody>, res: Response<{ ok: boolean }>) => {
      const { download_location } = req.body || {};
      if (!download_location || typeof download_location !== 'string') {
        return res.status(400).json({ error: 'Missing download_location in body' } as any);
      }

      // Only allow Unsplash URLs to avoid SSRF
      if (!download_location.startsWith('https://api.unsplash.com/')) {
        return res.status(400).json({ error: 'Invalid download_location' } as any);
      }
