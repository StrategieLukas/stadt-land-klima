<template>
  <div class="mt-4 py-4 p-4 mb-4">
    ...
    <div v-if="measure.translations?.[0]?.description_about || measure.description_about" class="mb-2 flex flex-row measure_ratings-start gap-4">
      <figure class="mt-0 flex shrink-0 flex-col">
        <img src="~/assets/icons/icon_info.svg" alt="" class="h-auto w-10 opacity-50" />
      </figure>
      <div class="has-long-links prose" v-html="sanitizeHtml(measure.translations?.[0]?.description_about || measure.description_about)" />
    </div>
  </div>

  <div v-if="measure.translations?.[0]?.description_evaluation_criteria || measure.description_evaluation_criteria" class="p-4 border-t-4 border-gray-300p-4 mb-4">
    <h3 class="mb-3 text-h3 font-bold text-black">
    ...
    <div class="mb-2 flex flex-row measure_ratings-start gap-4">
      ...
      <div class="has-long-links prose" v-html="measure.translations?.[0]?.description_evaluation_criteria || measure.description_evaluation_criteria" />
    </div>
  </div>
