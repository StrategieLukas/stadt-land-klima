<template>
  ...
  <div class="block lg:hidden project-page mt-4 bg-[#E8F9FD] shadow-md rounded-md overflow-hidden">
      ...
      <div class="p-6">
        ...
        <NuxtLinkLocale :to="backHref" class="text-blue-500 text-sm">← {{ backLabel }}</NuxtLinkLocale>
        <!-- Title and Subtitle -->
        ...
        <p class="text-sm text-gray-600 mb-1">
          <NuxtLinkLocale v-if="municipality_slug" :to="`/municipalities/${municipality_slug}`" class="text-blue-500 hover:underline">{{ location }}</NuxtLinkLocale>
    ...
    <div class="grid grid-cols-3 gap-6 prose max-w-none">
      ...
      <div class="flex flex-col text-sm text-gray-700 space-y-4 bg-white p-6 rounded-lg shadow-md">
        ...
        <p v-if="municipality_name" class="pb-2 border-b border-gray-300 flex justify-between">
          ...
          <NuxtLinkLocale v-if="municipality_slug" :to="`/municipalities/${municipality_slug}`" class="text-right text-blue-500 hover:underline">{{ municipality_name }}</NuxtLinkLocale>
